defmodule JumpstartAi.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%JumpstartAi.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(JumpstartAi.Accounts.User, id, authorize?: false) do
