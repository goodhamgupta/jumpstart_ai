defmodule JumpstartAi.HubSpotService do
  ...
  def fetch_contacts(user, opts \\ []) do
    ...
    with {:ok, access_token} <- get_valid_access_token(user) do
      ...
      properties = Keyword.get(opts, :properties, ["email", "firstname", "lastname", "phone", "company", "lifecyclestage"])
      
      Logger.info("HubSpot Service - Got valid access token, fetching contacts with limit #{limit}")
    ...
    with {:ok, access_token} <- get_valid_access_token(user) do
      ...
      case HTTPoison.post(url, body, headers) do
        ...
        {:ok, %HTTPoison.Response{status_code: status_code, body: response_body}} ->
          Logger.error("HubSpot - Contact creation failed with status #{status_code}: #{response_body}")
          {:error, {:api_error, status_code}}
  ...
  def update_contact(user, contact_id, contact_data) do
    with {:ok, access_token} <- get_valid_access_token(user) do
      ...
      case HTTPoison.patch(url, body, headers) do
        ...
        {:ok, %HTTPoison.Response{status_code: status_code, body: response_body}} ->
          Logger.error("HubSpot - Contact update failed with status #{status_code}: #{response_body}")
          {:error, {:api_error, status_code}}
  ...
  def search_contacts(user, query, opts \\ []) do
    with {:ok, access_token} <- get_valid_access_token(user) do
      ...
      properties = Keyword.get(opts, :properties, ["email", "firstname", "lastname", "phone", "company"])
      ...
      case HTTPoison.post(url, body, headers) do
        ...
        {:ok, %HTTPoison.Response{status_code: status_code, body: response_body}} ->
          Logger.error("HubSpot - Contact search failed with status #{status_code}: #{response_body}")
          {:error, {:api_error, status_code}}
        ...
        case Jason.decode(body) do
          {:ok, %{"results" => contacts} = response} ->
            ...
            case get_in(response, ["paging", "next", "after"]) do
              nil ->
                Logger.info("HubSpot Service - No more pages, returning #{length(all_contacts)} total contacts")
                {:ok, all_contacts}
  ...
  defp build_contacts_url(limit, properties, after_cursor) do
    ...
    query_params = if after_cursor do
  ...
  defp get_valid_access_token(user) do
    ...
    Logger.info("HubSpot Service - Has access token: #{not is_nil(user.hubspot_access_token)}")
    Logger.info("HubSpot Service - Token expires at: #{inspect(user.hubspot_token_expires_at)}")
    Logger.info("HubSpot Service - Token expired: #{token_expired?(user.hubspot_token_expires_at)}")
  ...
  defp refresh_hubspot_token(user) do
    ...
    if user.hubspot_refresh_token do
      ...
      case HTTPoison.post(url, body, headers) do
          ...
          case Jason.decode(response_body) do
            {:ok, %{"access_token" => access_token} = tokens} ->
              ...
              |> case do
                ...
                {:error, error} ->
                  Logger.error("HubSpot - Failed to update user with new tokens: #{inspect(error)}")
        ...
        {:ok, %HTTPoison.Response{status_code: status_code, body: response_body}} ->
          Logger.error("HubSpot - Token refresh failed with status #{status_code}: #{response_body}")
          {:error, :token_refresh_failed}
