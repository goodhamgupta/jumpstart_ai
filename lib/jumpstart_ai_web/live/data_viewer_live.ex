defmodule JumpstartAiWeb.DataViewerLive do
  ...
  defp load_data(socket) do
    ...
    {contacts_result, contacts_total} =
      load_contacts_paginated(user, socket.assigns.contacts_page, page_size)

    # Load contact notes with pagination
    {notes_result, notes_total} =
      load_contact_notes_paginated(user, socket.assigns.contact_notes_page, page_size)

    # Load emails with pagination
    {emails_result, emails_total} =
      load_emails_paginated(user, socket.assigns.emails_page, page_size)
    ...
    {ongoing_instructions_result, ongoing_instructions_total} =
      load_ongoing_instructions_paginated(
        user,
        socket.assigns.ongoing_instructions_page,
        page_size
      )
                ...
                <td class="px-6 py-4 text-sm text-gray-900">
                  <div class="max-w-lg truncate">
                    <%= if note.content && String.trim(note.content) != "" do %>
                      {String.slice(note.content, 0, 150)}{if String.length(note.content) > 150,
                        do: "...",
                        else: ""}
                    <% else %>
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
          <div>
            <p class="text-sm text-gray-700">
              ...
              to <span class="font-medium">{min(@current_page * @page_size, @total_records)}</span>
              of <span class="font-medium">{@total_records}</span>
              results
            </p>
          </div>
